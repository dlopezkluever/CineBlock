...
Phase 5 — Mannequin System: File Changes

  src/store.tsx — 4 new reducer actions:
  - ADD_MANNEQUIN / UPDATE_MANNEQUIN / REMOVE_MANNEQUIN — CRUD for placements keyed by   
  assetId+shotId
  - SET_ASSET_VISIBILITY — bulk-replace visibility map (used on shot switch)
  - TOGGLE_ASSET_VISIBILITY fixed to default visible (?? true) when key absent

  src/components/Mannequins.tsx — New file with 5 components:
  - CharacterMannequin — capsule body + sphere head, color-coded, renderOrder: 999 +     
  depthTest: false to render over splats
  - PropMannequin — box primitive, same render fixes
  - MannequinGizmo — wraps drei TransformControls, G/R/S keyboard shortcuts for mode     
  switching
  - PlacementRaycastHelper — crosshair cursor, raycasts collider mesh children, fallback 
  5 units forward, console logs for hit/miss diagnostics
  - MannequinScene — orchestrates all mannequins for active shot, filters by visibility, 
  handles selection/deselection

  src/views/StudioView.tsx — Major additions:
  - New "Assets" sidebar section between Active Shot and Captures: color dot, name, eye  
  toggle, Place/Remove buttons per asset
  - placingAssetId + selectedAssetId state for placement mode and gizmo selection        
  - MannequinScene added inside Canvas
  - Shot switch resets visibility to match shot's assetIds via SET_ASSET_VISIBILITY      
  - onPointerMissed on Canvas deselects mannequin
  - Placement mode indicator in top toolbar
